...
def search(request):
    articles_list = Article.objects.filter(
        article_title__icontains=request.GET['searchword']
        ).union(
        Article.objects.filter(
            article_title__icontains=request.GET['searchword'].upper()
        ),
        Article.objects.filter(
            article_title__icontains=request.GET['searchword'].capitalize()
        )
    )
...
# Article.objects.filter(article_title__icontains=('лгбт'.capitalize() or 'лгбт' or 'лгбт'.upper())
# Article.objects.filter(article_title__icontains=('лгбт'.upper() or 'лгбт' or 'лгбт'.capitalize()))